...
Each field can have the following features:
...
- If the command execution fails, the field will not have autocomplete functionality and the error will be logged.
- If the command execution takes longer than the timeout, the field will not have autocomplete functionality and the error will be logged.
- The command must return exit code 0 for a successful execution.

A command need to be provided and must return a list of suggestions in a JSON array format into the environment variable `TORII_AUTOCOMPLETE_OUTPUT_<FIELD-SLUG>`

#### Input

The input JSON format for the autocomplete command is as follows:

```json
{
  "value": "current value of the field",
  "output_env_var": "TORII_AUTOCOMPLETE_OUTPUT_<FIELD-SLUG>"
}
```

#### Output

The output JSON format for the autocomplete command is as follows:
...
```bash
export TORII_AUTOCOMPLETE_OUTPUT_MY_FIELD_SLUG='{ "results": ["This is value 1", "This is value 2", "This is value 3"] }'
```
...
if __name__ == '__main__':
    ...
    if len(input_value) > 3:
        ...
        output["results"] = ['This is value 1', 'This is value 2', 'This is value 3']

    # get the autocomplete env var key from the input json
    # which is in the format TORII_AUTOCOMPLETE_OUTPUT_<FIELD-SLUG>
    output_env_key = input_json['output_env_var']

    # set the results in the environment variable with the JSON result
    os.environ[output_env_key] = json.dumps(output)
```

:::note

`Select` and `Multi Select` fields have different autocomplete command output format. Please refer to the respective field types for more information.
